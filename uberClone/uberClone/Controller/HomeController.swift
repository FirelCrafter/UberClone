...
//  uberClone
//
//  Created by Михаил Щербаков on 21.03.2022.
//
...
class HomeViewController: UIViewController {
    ...
    private let actionButton: UIButton = {
        ...
        button.addTarget(HomeViewController.self, action: #selector(activationButtonPressed), for: .touchUpInside)
        return button
    }()
    ...
    @objc func activationButtonPressed() {
        switch actionButtonConfig {
        case .showMenu:
            delegate?.hamdleMenuToggle()
        ...
        PassengerService.shared.observeCurrentTrip { trip in
            ...
            case .completed:
                PassengerService.shared.deleteTrip { err, ref in
                    ...
                    self.configureActionButton(config: .showMenu)
                    self.inputActivationView.alpha = 1 
                    self.presentAlertController(withTitle: "Поездка завершена", message: "Спасибо, что пользуетесь нашим сервисом")
                }
            }
    ...
    func startTrip() {
        ...
        DriverService.shared.updateTripState(trip: trip, state: .inProgress) { err, ref in
            ...
            self.mapView.zoonToFit(annotations: self.mapView.annotations)
    ...
    func fetchDrivers() {
        ...
        PassengerService.shared.fetchDrivers(location: location) { (user) in
            ...
            var driverIsVisible: Bool {
                return self.mapView.annotations.contains { annotation -> Bool in
                    ...
                    if driverAnno.uid == user.uid {
                        driverAnno.updateAnnotationPositionCoordinate(withCoordinate: coordinate)
                        self.zoomForActiveTrip(withDriverUid: user.uid)
                        return true
                    }
                    return false
                }
    ...
    func observeCancelledTrip(trip: Trip) {
        DriverService.shared.observeTripCancelled(trip: trip) {
            ...
            self.animateRideActionView(shouldShow: false)
            self.centerMapOnUserLocation()
            self.presentAlertController(withTitle: "Поездка отменена", message: "Пассажир отменил текущую поездку")
        }
    }
    ...
    func configureLocationInputActivationView() {
        ...
        inputActivationView.anchor(top: actionButton.bottomAnchor, paddingTop: 12)
    ...
    func animateRideActionView(shouldShow: Bool, destination: MKPlacemark? = nil, config: RideActrionViewConfiguration? = nil, user: User? = nil) {
...
extension HomeViewController: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didStartMonitoringFor region: CLRegion) {
        if region.identifier == AnnotationType.pickup.rawValue {
            print("Monitoring pickup region")
        }
        
        if region.identifier == AnnotationType.destination.rawValue {
            print("Monitoring destination region")
        }
    }
...
private extension HomeViewController {
    ...
    func zoomForActiveTrip(withDriverUid uid: String) {
        ...
        self.mapView.zoonToFit(annotations: annotations)
    }
}
...
extension HomeViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, rendererFor overlay: MKOverlay) -> MKOverlayRenderer {
        if let route = self.route {
            ...
            let lineRenederer = MKPolylineRenderer(overlay: polyline)
            lineRenederer.strokeColor = .mainBlueTint
            lineRenederer.lineWidth = 4
            return lineRenederer
        }
...
extension HomeViewController: RideActionViewDelegate {
    ...
    func uploadTrip(_ view: RideActionView) {
        ...
        guard let destinationCoodrdinates = view.destination?.coordinate else { return }
        
        shouldPresentLoadingView(true, message: "Ищем авто...")
        
        PassengerService.shared.uploadTrip(pickupCoordinates, destinationCoodrdinates) { (err, ref) in
            if let err = err {
...
extension HomeViewController: PickupControllerDelegate {
    func didAcceptTrip(_ trip: Trip) {
        ...
        mapView.zoonToFit(annotations: mapView.annotations)
